...
int eap_dealer::start(std::vector<uint8_t> gateway_mac)
{
    ...
    {
        retry_times++;
        EAP_LOG_ERR("Failed to perform "
                    << "Start"
                    << ", retry times = " << retry_times << std::endl);
        EAP_LOG_INFO("Try to perform "
                     << "Start"
...
int eap_dealer::response_md5_challenge(std::vector<uint8_t> gateway_mac)
{
    ...
    if (ret)
    {
        ...
        if (eap_header->eap_code == 0x01) // Request
        {
            ...
            std::string notify(ntohs(eap_header->eap_length) - 5, 0); // 1 for NULL Terminator
            memcpy(&notify[0], ((uint8_t *)eap_header + 4 + 5),       // 4 - EAPol Header, 5 - EAP Header
                   ntohs(eap_header->eap_length) - 5);

            EAP_LOG_INFO("Gateway returns: Request, Notification: " << notify << std::endl);

            if (!notify.compare("userid error1"))
